...
#import "@preview/cetz-plot:0.1.3" as plt
...
#let cup(coor, size: (0.9, 1), name: none, ..styles, ratio: 100%) = {
  import draw: *
  styles = styles.named()
  styles = cetz.util.merge-dictionary(
    (
...
#let myrect(center, size: (1, 1), name: none, ..styles, label: []) = {
  import draw: *
  styles = styles.named()
  styles = cetz.util.merge-dictionary(
    (
...
#let mplot(
  ...
  draw.set-style(axes: (stroke: stroke, x: (label: (anchor: "west"))) + axis-style)
  plt.plot.plot(
    size: size,
    x-tick-step: x-step,
    y-tick-step: y-step,
...
#let floor(
  ...
  if not shadowed {
    line(name: name, start, (rel: (angle, length)), ..styles)
  } else {
    group(name: name, anchor: _anchor, padding: padding, {
      if flipped {
        scale(y: -100%)
      }
      let end = (rel: (length, 0), to: start)
      // let shadow-fill = tiling(
      //   size: (shadow-pitch, shadow-span)
      // )
      rotate(angle)
      line(name: "base", start, end, ..styles)
      let len = 0
      while len < length {
        let a = (to: start, rel: (len, 0))
        let b = (to: start, rel: (len + shadow-pitch, -shadow-span))
        line(a, b, stroke: if shadow-stroke == auto { 0.5pt } else { shadow-stroke })
        len += shadow-pitch
      }
      rect("base.start", (to: "base.end", rel: (0, -shadow-span)), fill: shadow-fill, stroke: none, name: "shadow")
    })
  }
}
...
#let content-line(
  ...
  let styling = coords.named()
  let coords = coords.pos()
  let prefix = "__phypic-help-anchors"

  // text node is in the form (body: [] | content, at: auto | coord, save: label)
  let anchors = coords.filter(c => not "body" in c and not "pos" in c)
  let draw-anchors = anchors.zip(range(anchors.len())).map(((pos, i)) => draw.anchor("p" + str(i), pos))

  draw.group(name: prefix, anchor: "default", {
    import draw: *
    set-origin((0, 0))
    anchor("default", (0, 0))
    draw-anchors.join()
  ...
  draw.get-ctx(ctx => {
    let prev = none // previous coordinate
    let next = none // next point coordinate
    let contents = () // the content nodes
    let lines = () // the lines
    let count = 0

    let anchors = range(anchors.len()).map(i => prefix + ".p" + str(i))
    ...
    for (i, coord) in coords.enumerate() {
      ...
      let dir = 0deg
      let body-angle = 0deg
      let base = base-vector

      if type(coord) == dictionary and "body" in coord {
        body = coord.remove("body")
        if "at" in coord {
          pos = coord.remove("at")
        } else {
          next = anchors.at(count, default: none)
          if next == none {
            pos = prev
            coord = (anchor: "west") + coord
          } else if prev == none {
            pos = next
            coord = (anchor: "east") + coord
          } else {
            pos = (prev, 50%, next)
            (ctx, prev, next, base) = cetz.coordinate.resolve(ctx, prev, next, base)

            import cetz.vector: *

            let v = sub(next, prev)
            let abs-v = len(v)
            let dot-product = dot(v, base)
            let cross-product = len(cross(v, base))
            let cosine = dot-product / abs-v
            let sine = cross-product / abs-v

            dir = calc.atan2(cosine, sine)
          }
          if "angle" in coord {
            body-angle = coord.remove("angle")
          } else {
            body-angle = dir
          }
          coord = (anchor: "south") + coord
        }

        contents.push({
          draw.content(
            pos,
            rotate(body-angle, body, reflow: true),
            angle: dir,
        ...
        // lines.push(coord)
        prev = anchors.at(count, default: none)
        count += 1
        // contents.push(draw.move-to(coord))
      }

      if type(coord) == dictionary and "name" in coord and "pos" in coord {
        let (name, pos) = coord
        contents.push(draw.anchor(name, pos))
      }
    }


    drawer(..anchors, ..styling, name: name)
    contents.join()
  })
}

#let arwtext(fill: pl.k, flip: false, ..args) = content-line(
  stroke: (paint: fill),
  mark: {
    let pos = if flip { "start" } else { "end" }
    (str(pos): "stealth", fill: fill)
...
#let valve(coor, size: (1, 0.5), name: none, anchor: "default", vertical: false) = {
  let (w, h) = size
  let one-w = w / 2
  draw.group(name: name, anchor: anchor, {
    import draw: *
    draw.anchor("default", coor)
    set-origin(coor)
    if vertical {
      rotate(90deg)
    }
    pline((0, 0), (rel: (-w / 2, -h / 2)), (rel: (0, h)), close: true)
    pline((0, 0), (rel: (w / 2, -h / 2)), (rel: (0, h)), close: true)
  })
}
...
#let spring(
  ...
  import cetz.decorations: zigzag
  import draw: *
  zigzag(line(start, end), ..styles)
}

#let filled-mark-line(start, end, ..points-styles) = {
  import draw: *
  let styles = points-styles.named()
  let get-stroke = cetz.util.resolve-stroke

  scope({
    let _ = styles.remove("name", default: none)
    let _ = styles.remove("mark", default: none)
    get-ctx(ctx => {
      let line-style = cetz.styles.resolve(
        ctx.style,
        root: "line",
        merge: styles,
      )
      set-style(mark: (
        fill: get-stroke(line-style.at("stroke", default: black)).paint,
      ))
      line(start, end, ..points-styles)
    })
  })
}
/// usage "<->" -> (start: "<", end: ">")
#let marken(string, ..styles) = {
  ...
  (mark: (start: reverse(start), end: end, ..styles.named()))
}

#canvas({
  import draw: *
  // spring((0, 0), (2, 0), amplitude: 0.5, gap: 0.2, name: "S")
  // line("S.start", "S.end")
  // // line((0, 1), (0, 2), mark: (end: ")>"))
  // set-style(line: (stroke: (paint: red)))
  // // set-style(line: (stroke: (thickness: 2pt)))
  filled-mark-line((0, 4), (3, 4), (3, 2), mark: (end: ")>"), name: "ex")
  circle("ex.end")
  // line((0, 1), (0, 2), ..marken("-<>"))
  // incline((), angle: 60deg, length: 4)
  // incline-sides((0, 0), size: (3, 4))
  floor((0, 0), shadowed: true, name: "F")
  circle("F.base.end")

})


#let mline = filled-mark-line

#let measurement = content-line.with(mark: (symbol: "|"))